...
pair_mask = []
for refe in tqdm(curr_lib.coordinates):
    curr_pair_mask = conformer_mask_from_crmsd(reference=refe, fragment_library=prev_lib, conformer_rmsd_threshold=cRMSD).astype(bool)
    pair_mask.append(curr_pair_mask)
...
for chunk in tqdm(list(range(nchunks))):
    ...
    pair_mask_filter = pair_mask[curr_curr_poses["conformer"][col1], prev_poses["conformer"][col2]]
    result = np.stack((col1, col2),axis=1)
    candidates = result[pair_mask_filter]
