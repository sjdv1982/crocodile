...
for subchain in subchains:
    ...
    for fragmentnr, fragment in enumerate(reversed(subchain)):
        ...
        enes = np.loadtxt(os.path.join(direc, "poses-filtered.ene"), ndmin=1)
        ...
        if fragmentnr == 0:
            ...
            all_enes = np.empty((len(poses), len(subchain)))
    ...
    subchain_results.append(
        (all_poses, all_rmsds, all_enes, first_pose_mapping, pose_inds)
    )
...
for fragment, fragdirs in merged_fragments.items():
    ...
    enes = np.loadtxt(os.path.join(fragdirs[0], "poses-filtered.ene"), ndmin=1)
    ...
    for direc, sub in zip(fragdirs, (sub1, sub2)):
        ...
        halfposes_ene = sub[2][subpose_inds - 1]
        all_halfposes_ene.append(halfposes_ene)
    ...
    new_ene = np.concatenate(all_halfposes_ene, axis=1)
    subchains.append(subchains[subch1] + [fragment] + subchains[subch2][::-1])
    new_pose_inds = sub1[4][all_subposes_inds[0] - 1] if sub1[4] is not None else None
    subchain_results.append((new_poses, new_rmsd, new_ene, sub2[3], new_pose_inds))
        ...
        for s2, subchain2 in enumerate(list(subchains)):
            ...
            if subchain1[0] == subchain2[0]:
                ...
                for sub_ind in sub_inds:
                    ...
                    ene1block = sub1[2][mask1]
                    ene2block = sub2[2][mask2]
                    inter_ene = ene1block[0, 0]
                    assert inter_ene == ene2block[0, 0]
                    if len(ene1block) > 1:
                        assert inter_ene == ene1block[1, 0]
                    new_ene = np.empty(new_poses.shape)

                    ene1block_rev = ene1block[:, ::-1]
                    new_ene[:, : cols1 - 1] = ene1block_rev[:, :cols1][inds1][:, :-1]
                    new_ene[:, cols1 - 1] = inter_ene
                    new_ene[:, -cols2 + 1 :] = ene2block[:, -cols2:][inds2][:, 1:]

                    all_new_poses.append(new_poses)
                    all_new_rmsd.append(new_rmsd)
                    all_new_ene.append(new_ene)

                all_new_poses = np.concatenate(all_new_poses)
                all_new_rmsd = np.concatenate(all_new_rmsd)
                all_new_ene = np.concatenate(all_new_ene)
                new_subchain = (
                    list(reversed(subchain1)) + subchain2[1:]
                )  # ascending order
                subchains.append(new_subchain)
                subchain_results.append(
                    (all_new_poses, all_new_rmsd, all_new_ene, None)
                )
...
for pos in range(last_frag - first_frag + 1):
    ...
    np.savetxt(os.path.join(output_directory, f"frag{frag}.ene"), frag_ene, fmt="%.3f")
