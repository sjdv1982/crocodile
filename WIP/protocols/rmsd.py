...
poses_rmsd_file = os.path.join(args.target_directory, "poses.rmsd")
frag_file = os.path.join(args.target_directory, "FRAG")
if not os.path.exists(args.target_directory):
    err(f"'{args.target_directory}' does not exist")
elif not os.path.exists(poses_file):
    err(f"poses.npy in '{args.target_directory}' does not exist")
elif not os.path.exists(frag_file):
    err(f"file FRAG in '{args.target_directory}' does not exist")

fragment = int(open(frag_file).read())
...
np.savetxt(poses_rmsd_file, rmsd, fmt="%.3f")
