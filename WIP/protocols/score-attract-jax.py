...
assert lig_atomtypes00.ndim == 1 and len(lig_atomtypes00) == lig.shape[-2], (
    len(lig_atomtypes00),
    lig.shape,
)
...
for field in grid._fields:
    ...
    d[field] = value
d["neighbour_grid_ravel"] = d["neighbour_grid"].reshape(
...
def nb_energy_single(
    ...
    dsq = (d * d).sum()
    return cond(
        ((receptor_atom < 2**16 - 1) & (dsq < plateaudissq)),
        nonbon_dif,
        lambda *args: 0.0,
        d,
        ff,
...
nb_energy_vec = jnp.vectorize(
    nb_energy_single, excluded=(1, 4, 5, 6, 7, 8), signature="(),(),()->()"
)
...
def potential_energrad_voxel(
    ...
    def inner():
        return eval_potential_grid(
            lig_atomtype_pos, vox_innergrid, inner_potential_grid
        )

    def outer():
        return eval_potential_grid(
            lig_atomtype_pos, vox_outergrid, outer_potential_grid
        )
...
potential_energrad_all = jnp.vectorize(
    potential_energrad_voxel, excluded=(5, 6), signature="(),(m),(),(m),()->(3)"
)
...
def potential_energrad(all_coors_lig, lig_atomtype_pos, grid):
    ...
    atom_energrads = potential_energrad_all(
        lig_atomtype_pos,
        vox_innergrid,
        in_innergrid,
        vox_outergrid,
        in_outergrid,
        grid.inner_potential_grid,
        grid.outer_potential_grid,
    )
...
def potential_energy(mat, coor_lig, conformers, lig_atomtype_pos, grid):
    ...
    all_coors_lig = jnp.einsum(
        "ijk,ikl->ijl", coor_lig2[conformers], mat
    )  # diagonally broadcasted form of coor_lig.dot(mat)
...
def generate_ind_innergrid(all_coors_lig, grid, grid_dim):
    ...
    pos_innergrid = jnp.where(
        in_innergrid[:, :, None],
        jnp.floor(vox_innergrid + 0.5).astype(np.int32),
        out_of_bounds,
    )
    ind_innergrid = jnp.ravel_multi_index(
        (pos_innergrid[:, :, 0], pos_innergrid[:, :, 1], pos_innergrid[:, :, 2]),
        dims=grid_dim,
        mode="clip",
    )
    ind_innergrid = ind_innergrid + out_of_bounds**3 * (
        1 - in_innergrid.astype(np.uint8)
    )
...
if CALC_GRADS:
    ...
    (_, energies), gradients = vgrad_main(
        mat[:BATCH],
        coor_rec,
        rec_atomtypes,
        coor_lig,
...
for it in range(2):
    ...
    for offset in range(0, len(mat), BATCH):
        ...
        if CALC_GRADS:
            (total_energy0, energies_batch), gradients_batch = vgrad_main(
                mat_batch,
                coor_rec,
                rec_atomtypes,
                coor_lig,
            ...
            total_energy0, energies_batch = main(
                mat_batch,
                coor_rec,
                rec_atomtypes,
                coor_lig,
