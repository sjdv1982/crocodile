#!/usr/bin/env python3
"""Trace pose-pose superposition implementation.
...
This is computed with the trace SD identity instead of explicitly materializing
the rotated centered X-Y poses.
...
DEFAULT_OUTPUT_ROOT = Path(__file__).with_name("trace_pose_dirs")
DEFAULT_SUMMARY_CSV = Path(__file__).with_name("trace_pose_pose_summary.csv")
...
def _rotaconformer_sd_from_trace(
    candidate_trace: float,
    reference_trace: float,
    cross_second_moment: np.ndarray,
    rotamers: np.ndarray,
) -> np.ndarray:
    trace_terms = np.einsum(
        "nij,ij->n",
        rotamers,
        cross_second_moment,
        optimize=True,
    )
    sd = candidate_trace + reference_trace - 2.0 * trace_terms
    return np.maximum(sd, 0.0).astype(np.float32, copy=False)
...
def _write_case_poses(
    ...
    reference_trace = float(np.einsum("ij,ij->", reference_centered, reference_centered))
    ...
    try:
        for conformer, crmsd in zip(case.candidate_conformers, case.candidate_crmsds):
            ...
            candidate_trace = float(np.einsum("ij,ij->", centered, centered))
            cross_second_moment = centered.T.dot(reference_centered)
            ...
            for rotamer_start in range(0, len(rotamers), ROTAMER_BATCH_SIZE):
                ...
                rotaconformer_sd = _rotaconformer_sd_from_trace(
                    candidate_trace,
                    reference_trace,
                    cross_second_moment,
                ...
                rot_boundary = (
                    np.abs(rotaconformer_sd - remaining2) <= rot_boundary_tolerance
                )
                if np.any(rot_boundary):
                    rot_boundary_rows = np.nonzero(rot_boundary)[0]
                    boundary_centered_poses = np.einsum(
                        "aj,njk->nak",
                        centered,
                        rotamer_block[grid_keep][rot_boundary_rows],
                    )
                    boundary_centered_dif = (
                        boundary_centered_poses
                        - reference_centered[None, :, :]
                    )
                    rotaconformer_sd[rot_boundary_rows] = np.einsum(
                        "nij,nij->n",
                        boundary_centered_dif,
                        boundary_centered_dif,
                    )
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the trace SD-math pose-pose superposition implementation."
    )
    parser.add_argument(
        "--limit",
        type=int,
