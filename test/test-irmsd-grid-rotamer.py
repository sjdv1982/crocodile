...
ligand = parse_pdb(open("1AVXB-unbound-heavy.pdb").read())
ligand_coor = get_coor(ligand)
ligand_coor -= ligand_coor.sum(axis=0)/len(ligand_coor)
refe_receptor = parse_pdb(open("1AVXA-bound-heavy.pdb").read())
refe_ligand = parse_pdb(open("1AVXB-bound-heavy.pdb").read())

matrices = np.load("../test-roc/rocs.npy")
...
if_r = get_coor(receptor[imasks[0]])
...
for mnr, m in enumerate(matrices):
    lig_rot = ligand_coor.dot(m)
    if_l = lig_rot[imasks[1]]
    ...
    print(mnr+1, "Nodes:", len(accept), "leaves:", len(accept_lowest), "total:", sum([8**(a[0]-1) for a in accept])+len(accept_lowest))